...
namespace CW_Devyatov_238 {
    ...
    public class HandPointer : MonoBehaviour{
        ...
        private bool RoutineInProgress;
         ...
         void OnEnable() {
		    HealthController.OnUnitDeath += HandPointerCheck; //subscribe to event
            GetComponent<Image>().enabled = false; //disable by default
	    }

	    void OnDisable() {
		    HealthController.OnUnitDeath -= HandPointerCheck; //unsubscribe to event
	    }
        ...
        IEnumerator FlickerHand(){
            ...
            for(int i=0; i<8; i++){
                ...
                if(hand.enabled) CW_Devyatov_238.AudioController.PlaySFX(sfx, Camera.main.transform.position);
