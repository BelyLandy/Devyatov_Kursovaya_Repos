...
namespace CW_Devyatov_238 {
    ...
    public enum UNITTYPE { PLAYER = 0, ENEMY = 10}
    ...
    public class UnitSettings : MonoBehaviour {
        ...
        [HideInInspector] public bool targetInSight; //true if the target is in the field of view of this enemy
        ...
        private void OnValidate()
        {
	        ...
	        // 1. Face the whole character
	        transform.localRotation = toLeft
		        ? Quaternion.Euler(0f, 180f, 0f)
		        : Quaternion.identity;

	        // 2. Face the HealthBar child (if present)
	        foreach (Transform child in transform)
	        {
		        if (child.CompareTag("HealthBar"))       // safer/faster than child.tag == "HealthBar"
		        {
			        child.localRotation = toLeft
				        ? Quaternion.Euler(0f, 180f, 0f)
				        : Quaternion.identity;

			        break;                               // Health bar is unique → stop looping
		        }
	        }
        }
