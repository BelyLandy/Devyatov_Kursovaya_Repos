...
public class HealthController : MonoBehaviour
{
    ...
    [Header("Звуковые эффекты")] public string SFXOnHit = "";
    public string SFXOnDestroy = "";

    [Header("Эффекты")] public bool EnableHitFlash = true;
    public float HitFlashDuration = 0.15f;
    private bool hitFlashActive;

    [Space(10)] public bool EnableShakeEffect;
    public float ShakeIntensity = 0.08f;
    public float ShakeDuration = 0.5f;
    public float ShakeSpeed = 50f;

    [Space(10)] public GameObject HitEffectPrefab;
    ...
    void InitializeMiniHealthBar()
    {
        if (healthBarInstance == null)
        {
            ...
            healthBarInstance.transform.GetChild(0).transform.localScale =
                new Vector3((float)CurrentHealth / MaxHealth, 1, 1);
            //healthBarInstance.transform.localRotation = Quaternion.Euler(0,0,90);
        }
    }
    ...
    public void SubtractHealth(int damage)
    {
        ...
        if (CurrentHealth > 0)
            AudioController.PlaySFX(SFXOnHit, transform.position);
        else
            AudioController.PlaySFX(SFXOnDestroy, transform.position);
