...
namespace CW_Devyatov_238 {
    ...
    public class UnitActions : MonoBehaviour {
        ...
        // Turn the sprite (and its HealthBar child) so it always faces the target.
        public void TurnToTarget()
        {
            ...
            // 1. Face the character toward the target
            transform.localRotation = toLeft
                ? Quaternion.Euler(0f, 180f, 0f)
                : Quaternion.identity;

            // 2. Give the HealthBar child the same facing
            foreach (Transform child in transform)               // direct children only
            {
                if (child.CompareTag("HealthBar"))               // fast & safe tag check :contentReference[oaicite:0]{index=0}
                {
        ...
        public void TurnToDir(DIRECTION dir)
        {
            ...
            // 2. Rotate the child tagged “HealthBar” (if any)
            foreach (Transform child in transform)           // iterate only direct children
            {
                if (child.CompareTag("HealthBar"))
                {
                    ...
                    break;   // leave the loop – we found our bar
                }
            }
        }
       ...
       public void MoveToVector(Vector2 moveDir, float moveSpeed){
        ...
        public void JumpSequence() {
            ...
            // Получаем входной вектор (X и Y)
            Vector2 inputVector = InputManager.GetInputVector();
    
            // Горизонтальное перемещение (без изменений)
            if (inputVector.x != 0)
                TurnToDir(inputVector.x > 0 ? DIRECTION.RIGHT : DIRECTION.LEFT);
            moveVector.x = transform.position.x + (inputVector.x * settings.moveSpeedAir * Time.fixedDeltaTime);
    
            // Вертикальное перемещение
            // 1. Физика прыжка: сила прыжка и гравитация
            moveVector.y += yForce * Time.fixedDeltaTime * settings.jumpSpeed;
            yForce -= settings.jumpGravity * Time.fixedDeltaTime * settings.jumpSpeed;
    
            // 2. Дополнительное вертикальное управление в воздухе
            moveVector.y += inputVector.y * settings.airControlVerticalSpeed * Time.fixedDeltaTime;
    
            // Обновление groundPos:
            // Прибавляем к groundPos небольшое значение, зависящее от вертикального ввода.
            // Это позволяет изменять базовую высоту приземления во время прыжка.
            groundPos += inputVector.y * settings.groundAdjustmentFactor * Time.fixedDeltaTime;
    
            // Обновляем смещение коллайдера так, чтобы он соответствовал разнице между текущей позицией и groundPos
            if (col2D)
                col2D.offset = new Vector2(col2D.offset.x, -(moveVector.y - groundPos));
        ...
        public void PlaySFX(string sfx){
            CW_Devyatov_238.AudioController.PlaySFX(sfx, transform.position);
        }
        ...
        public void Footstep(){
            ...
            AudioController.PlaySFX("FootstepGrass", transform.position);
        }
