...
public class EnemyKeepDistance : State
{
    ...
    public override void Enter()
    {
        if (!unit.target)
        {
            ...
            return;
        } //go back to idle if there is no target
        ...
        float xPos = unit.target.transform.position.x +
                     (distance.x * -(int)unit.dir); //unit.dir is used to keep the enemy on the same side of the target
