...
public class TemplateReviewPanel : MonoBehaviour
{
    ...
    public void Awake()
    {
        ...
        {
            new TMP_Dropdown.OptionData(BaseRecognizeAlgorithm.Step.RAW.ToString()),
            new TMP_Dropdown.OptionData(BaseRecognizeAlgorithm.Step.RESAMPLED.ToString()),
            new TMP_Dropdown.OptionData(BaseRecognizeAlgorithm.Step.ROTATED.ToString()),
            new TMP_Dropdown.OptionData(BaseRecognizeAlgorithm.Step.SCALED.ToString()),
            new TMP_Dropdown.OptionData(BaseRecognizeAlgorithm.Step.TRANSLATED.ToString())
        };
    ...
    {
        UpdateTemplates();
        ShapeRecognizerManager.GestureTemplate[] candidates = _templates.GetRawTemplatesByName(_currentTemplateName);
        if (candidates.Length > 0)
        {
            _displayTemplate.Draw(candidates[_currentTemplateIndex], (BaseRecognizeAlgorithm.Step) _step);
        }
        else
        {
    ...
    {
        IEnumerable<ShapeRecognizerManager.GestureTemplate> templatesByName = _templates.ProceedTemplates
            .Where(template => template.Name == _currentTemplateName).ToList();
        ShapeRecognizerManager.GestureTemplate templateToRemove = templatesByName
