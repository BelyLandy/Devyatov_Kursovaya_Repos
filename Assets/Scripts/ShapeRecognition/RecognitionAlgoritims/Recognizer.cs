    ...
    public DollarPoint[] ResamplePoints(DollarPoint[] points, int n)
{
    ...
    if (pathLength == 0)
    {
        // Если длина пути равна 0, возвращаем исходные точки
        return points;
    }

    float incrementValue = pathLength / (n - 1);
    ...
    for (int i = 1; i < points.Length; i++)
    {
        ...
        if (previousDollarPoint.StrokeIndex == currentDollarPoint.StrokeIndex)
        {
            ...
            // Если расстояние равно 0, пропускаем итерацию, чтобы избежать деления на ноль
            if (distance == 0)
                continue;
            ...
            if (proceedDistance + distance >= incrementValue)
            {
                while (proceedDistance + distance >= incrementValue)
                {
                    ...
                    float approximatedX = previousDollarPoint.Point.x +
                                            t * (currentDollarPoint.Point.x - previousDollarPoint.Point.x);
                    float approximatedY = previousDollarPoint.Point.y +
