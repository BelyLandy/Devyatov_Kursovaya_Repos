...
public class BaseRecognizeAlgorithm : Recognizer, IRecognizer
{
        ...
        float angle)
    {
        ...
        ShapeRecognizerManager.GestureTemplate bestTemplate = new ShapeRecognizerManager.GestureTemplate();

        // Приводим шаблоны к обработанному виду для сравнения
        IEnumerable<ShapeRecognizerManager.GestureTemplate> proceedGestures = gestureTemplates.Select(template =>
            new ShapeRecognizerManager.GestureTemplate() { Points = Normalize(template.Points, 64), Name = template.Name });

        foreach (ShapeRecognizerManager.GestureTemplate gestureTemplate in proceedGestures.Where(template =>
    ...
    private float DistanceAtBestAngle(DollarPoint[] points, ShapeRecognizerManager.GestureTemplate template, float thetaA,
        float thetaB,
    ...
    private float DistanceAtAngle(DollarPoint[] points, ShapeRecognizerManager.GestureTemplate template, float angle)
    {
        List<DollarPoint> newPoints = RotateBy(points, angle);
